...
class SecureVaultBackground {
    ...
    handleNotificationClick(notificationId) {
        if (notificationId.startsWith('securevault-notification-')) {
        ...
        try {
            switch (message.action) {
                ...
                case 'credentialsDetected':
                    this.pendingCredentials = message.credentials;
                    this.showNotification('Credentials detected for ' + message.credentials.site + '. Click to save.');
                    sendResponse({ success: true });
                    break;

                case 'getPendingCredentials':
                    sendResponse({ credentials: this.pendingCredentials });
    ...
    async getCredentialsForSite(domain) {
        try {
            const result = await chrome.storage.local.get(['vaultKey']);
            if (!result.vaultKey) {
                return [];
            }

            const token = result.vaultKey;

            // 1. Try Native Host first for independence
            try {
                const response = await new Promise((resolve, reject) => {
                    chrome.runtime.sendNativeMessage('com.notionvault.passwords', {
                        command: 'fetch',
                        key: token
                    }, (res) => {
                ...
                if (response && response.entries) {
                    return response.entries.filter(e =>
                        (e.site || '').toLowerCase().includes(domain.toLowerCase()) ||
                        domain.toLowerCase().includes((e.site || '').toLowerCase())
                    );
                }
            } catch (nativeError) {
                console.log('Native host not available, trying API...');
            }
            ...
            try {
                ...
                console.error('API not available or timeout');
            }

            return [];
        } catch (error) {
            console.error('Error fetching credentials in background:', error);
            return [];
        }
    }
